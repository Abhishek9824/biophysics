...
I started the [Helion] project, a Python-embedded DSL for writing
high-performance ML kernels that compiles to Triton and integrates tightly
with PyTorch’s compiler stack. Helion raises the abstraction above
Triton by automatically generating and autotuning hundreds of candidate
implementations from a single kernel, making it easier to author correct
kernels that remain performant across hardware generations. For more,
see the [Helion blog post].
